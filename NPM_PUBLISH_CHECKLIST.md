...
- [ ] Rebuild and smoke-test the actual packaged executable.
- [x] The current `app/dist/bin.js --help` output is older than `app/src/controllers/help.ts`, proving that the shipped CLI is stale.
- [x] Fix `.gitignore` verification in `verifyGitignore.ts`. It concatenates `currentpath + ".gitignore"`; because the CLI passes a path without a trailing slash, it looks beside the project directory instead of inside it. Use `path.join(currentpath, ".gitignore")` and add a regression test.
- [x] Restrict the published files with a `files` allowlist in `package.json` (preferably `app/dist`, `README.md`, `LICENSE`, and optionally `.env.example`). The current dry run packs 102 files, including TypeScript source, tests, `tsconfig.json`, `dutoaocs.config.json`, and stale build artifacts.
- [x] Add a real `LICENSE` file or change the `license` field. `package.json` declares MIT, but no license file exists.
...
- [x] Replace the live OpenAI call in `verifyOpenAiApi.test.ts` with a mocked client. The current test requires a real secret, spends API quota, and can fail because of network/service state. Also make the dotenv test set and restore its own test environment variable.
...
- [x] Add useful npm `keywords`, and consider `publishConfig: { "access": "public" }` to make public-package intent explicit.
