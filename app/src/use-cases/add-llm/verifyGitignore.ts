...
export default function verifyGitignore(currentpath:string):boolean{
    if (!fs.existsSync(currentpath+".gitignore")){
        console.log("Can not fund .gitignore")
    ...
    if (!gitignore_data.includes(".env")){
        console.log("'.env' not found in your .gitignore")
        console.log("It necessary for it to be in there to continue")
        console.log("Rolling back .env change. Re-add your OpenAi API key again, and re-run 'dutoaocs add-llm --verify'")
        rollbackDotenv()
